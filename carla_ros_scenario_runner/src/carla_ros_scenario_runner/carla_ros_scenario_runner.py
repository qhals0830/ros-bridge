...
try:
    ...
    print("ERROR: CARLA Python Egg not available. Please add <CARLA_DIR>/PythonAPI/carla/dist/carla-<CARLA_VERSION>-py<PYTHON_VERSION>-linux-x86_64.egg to your PYTHONPATH.")
    sys.exit(1)

try:
    from agents.navigation.local_planner import LocalPlanner
except ImportError:
    print("ERROR: CARLA Python Agents not available. Please add <CARLA_DIR>/PythonAPI/carla to your PYTHONPATH.")
    sys.exit(1)
...
class CarlaRosScenarioRunner(object):
    ...
    def __init__(self, role_name, host, scenario_runner_path):
        ...
        self._scenario_runner = ScenarioRunnerRunner(scenario_runner_path, host, self.scenario_runner_status_updated, self.scenario_runner_log)
        self._request_queue = Queue()
        self._execute_scenario_service = rospy.Service('/scenario_runner/execute_scenario', ExecuteScenario, self.execute_scenario)

    def scenario_runner_log(self, log):
        rospy.logwarn(log)
    ...
    def execute_scenario(self, req):
        ...
        if not os.path.isfile(req.scenario.scenario_file):
            rospy.logwarn("Requested scenario file not existing {}".format(req.scenario.scenario_file))
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            if current_req:
                ...
                #execute scenario
                scenario_executed = self._scenario_runner.execute_scenario(current_req.scenario_file)
